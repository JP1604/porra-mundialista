...
/**
 * Plantilla de columnas COMPARTIDA entre header y filas.
 * Anchos fijos en las columnas numéricas → los valores siempre caen
 * exactamente bajo su título (con `auto` cada grid calculaba anchos distintos).
 *   base : # | jugador | pts
 *   sm   : + exactos + aciertos
 *   md   : + bonus
 */
...
export function LeaderboardTable({
  ...
  return (
    ...
    >
      {/* Header */}
      <div role="row" className={cn(GRID_COLS, 'py-2.5 bg-white/[0.03] border-b border-white/[0.06]')}>
          ...
          >
            ...
            <div role="cell" className="flex items-center justify-center">
              {rankCfg ? (
                ...
                <span className="text-slate-500 font-mono text-sm tabular-nums">{entry.rank}</span>
              )}
            </div>
            ...
            <div role="cell" className="flex items-center gap-2.5 min-w-0">
              <Avatar className="w-7 h-7 shrink-0">
                <AvatarFallback className={cn(
                  'text-[10px] font-bold',
                  rankCfg?.avatar ?? (isMe ? 'bg-[#00D084]/15 text-[#00D084]' : 'bg-white/[0.07] text-slate-400')
                )}>
              ...
              <span className={cn(
                ...
                isMe ? 'text-[#00D084]' : entry.rank <= 3 ? 'text-white' : 'text-slate-300'
              )}>
                {entry.alias}
            ...
            <span role="cell" className={cn(
              ...
              entry.rank === 1 ? 'text-[#FFD700]' : 'text-white'
            )}>
              {entry.total_points}
            </span>
